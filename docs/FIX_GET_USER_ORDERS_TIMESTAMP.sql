-- Fix the get_user_orders function with correct timestamp type
-- The error shows order_created_at is TIMESTAMPTZ in the database

-- First drop the existing function
DROP FUNCTION IF EXISTS get_user_orders(UUID);

-- Recreate with correct timestamp type (TIMESTAMPTZ instead of TIMESTAMP)
...
RETURNS TABLE (
    ...
    tracking_url TEXT,
    total_price NUMERIC,
    order_created_at TIMESTAMPTZ,  -- Changed from TIMESTAMP to TIMESTAMPTZ
    items JSONB
) 
LANGUAGE plpgsql
SECURITY DEFINER
