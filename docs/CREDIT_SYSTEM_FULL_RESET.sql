-- COMPLETE CREDIT SYSTEM RESET AND SETUP
-- This script handles existing functions and missing columns

-- STEP 1: Drop ALL existing credit-related functions
...
-- STEP 2: Drop views that depend on the tables
DROP VIEW IF EXISTS public.user_credit_balance;

-- STEP 3: Check and add missing columns to credits table if it exists
DO $$ 
BEGIN
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'credits') THEN
        ...
        -- First, let's see what transaction types exist in the data
        -- This will help us understand what types are currently in use
        
        -- Update any non-standard transaction types to standard ones
        UPDATE public.credits 
        SET transaction_type = 'used' 
        WHERE transaction_type NOT IN ('earned', 'used', 'adjusted', 'expired', 'reversed', 'deduction_pending_payment');
        
        -- Now update transaction_type constraint to include new types
        ALTER TABLE public.credits DROP CONSTRAINT IF EXISTS credits_transaction_type_check;
        ALTER TABLE public.credits ADD CONSTRAINT credits_transaction_type_check 
            CHECK (transaction_type IN ('earned', 'used', 'adjusted', 'expired', 'reversed', 'deduction_pending_payment'));
    ELSE
...
-- STEP 5: Create credit_notifications table if it doesn't exist
...
-- STEP 7: Now create the view with all columns available
