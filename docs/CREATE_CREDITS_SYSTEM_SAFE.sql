-- Create Credits System for Sticker Shuttle (Safe Version)
...
CREATE TABLE IF NOT EXISTS credits (
    ...
    reason TEXT,
    transaction_type TEXT NOT NULL CHECK (transaction_type IN ('add', 'deduct', 'used')),
    order_id UUID, -- References orders_main(id) but not enforced to avoid circular dependencies
...
DROP POLICY IF EXISTS "Users can view own notifications" ON credit_notifications;
DROP POLICY IF EXISTS "Users can update own notifications" ON credit_notifications;
