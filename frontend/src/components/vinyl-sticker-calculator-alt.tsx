...
export default function StickerCalculatorAlt({ initialBasePricing, realPricingData }: StickerCalculatorProps) {
  ...
  const updatePricing = useCallback(() => {
    ...
    if (area > 0 && quantity >= 15) {
      const { total, perSticker } = calculatePrice(quantity, area, false)
  ...
  }, [selectedSize, customWidth, customHeight, selectedQuantity, customQuantity])
      ...
      product: {
        ...
        updatedAt: new Date().toISOString(),
          ...
          selections: {
            ...
            proof: { type: "finish" as const, value: true, displayValue: "Send Proof", priceImpact: 0 },
            rush: { type: "finish" as const, value: false, displayValue: "Standard", priceImpact: 0 },
      ...
      <div className="">
        ...
        <div className="rounded-3xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 mb-4 lg:mb-6">
            {/* Left Column - Contains Material, Size, and Upload/Instructions */}
            <div className="space-y-4">
              {/* Material and Size Stacked */}
              <div className="space-y-4">
                {/* Material Selection */}
                <div>
                  ...
                  <div className="container-style p-3 lg:p-4 transition-colors duration-200 h-full">
                  <div className="space-y-2">
                    <button
                      onClick={() => setSelectedMaterial("Matte")}
                      className={`button-interactive relative w-full text-left px-3 py-2 rounded-lg flex items-center gap-2 transition-all border backdrop-blur-md text-sm
                        ${
                    ...
                    >
                      Matte
                      <span className="absolute top-0.5 right-1 text-[9px] text-green-300 font-medium">
                        Most Popular
                      </span>
                    </button>
                    ...
                    <button
                      onClick={() => setSelectedMaterial("Gloss")}
                      className={`button-interactive relative w-full text-left px-3 py-2 rounded-lg flex items-center gap-2 transition-all border backdrop-blur-md text-sm
                        ${
                          selectedMaterial === "Gloss"
                            ? "bg-green-500/20 text-green-200 font-medium border-green-400/50 button-selected animate-glow-green"
                            : "hover:bg-white/10 border-white/20 text-white/80"
                        }`}
                    >
                  ...
                  <div className="container-style p-3 lg:p-4 transition-colors duration-200 h-full">
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['Small (2")', 'Medium (3")', 'Large (4")', 'X-Large (5")'].map((size) => (
                          <button
                            ...
                            className={`button-interactive relative w-full text-left px-3 py-2 rounded-lg flex items-center justify-center transition-all border backdrop-blur-md text-sm
                              ${
                                selectedSize === size
                          ...
                          >
                            <span>{size}</span>
                            {size === 'Medium (3")' && (
                              <span className="absolute top-0.5 right-1 text-[9px] text-purple-300 font-medium">
                                Most Popular
                              </span>
                            )}
                          </button>
                      ...
                      <button
                        ...
                        className={`button-interactive relative w-full text-left px-3 py-2 rounded-lg flex items-center justify-between transition-all border backdrop-blur-md text-sm
                          ${
                      ...
                      <div className="mt-2 space-y-2">
                        <div className="flex gap-2">
                          <input
                            ...
                            step="0.1"
                            className="w-1/2 px-2 py-1.5 rounded-lg border border-white/20 bg-white/10 text-white placeholder-white/60 focus:outline-none focus:border-purple-400 backdrop-blur-md button-interactive text-sm [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                          />
                          <input
                            type="number"
                            placeholder="H"
              ...
              {/* Bottom Row - Upload/Instructions */}
              <div className="container-style p-3 lg:p-4 transition-colors duration-200 h-fit">
                ...
                <div>
                  ...
                  {!uploadedFile ? (
                    <div 
                      className="border-2 border-dashed border-white/20 rounded-lg p-4 text-center hover:border-purple-400 transition-colors cursor-pointer backdrop-blur-md relative"
                      onDrop={handleDrop}
                    ...
                    >
                      ...
                      {isUploading ? (
                        <div className="mb-2">
                          ...
                          <p className="text-white font-medium text-sm mb-1">Uploading...</p>
                          {uploadProgress && (
                          ...
                          {uploadProgress && (
                            <p className="text-white/80 text-xs">{uploadProgress.percentage}% complete</p>
                          )}
                        </div>
                      ) : (
                        <div className="mb-2">
                          ...
                          <p className="text-white font-medium text-sm mb-1 hidden md:block">Drag or click to upload</p>
                          <p className="text-white font-medium text-sm mb-1 md:hidden">Tap to add file</p>
                          <p className="text-white/80 text-xs">All formats supported</p>
                        </div>
                      )}
                    </div>
                  ) : (
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-green-200 font-medium break-words text-sm truncate">{uploadedFile.original_filename}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <button
                            onClick={() => document.getElementById('file-input')?.click()}
                            className="text-blue-300 hover:text-blue-200 p-1 hover:bg-blue-500/20 rounded transition-colors cursor-pointer text-sm"
                            title="Replace file"
                          >
                            🔄
                          </button>
                          <button
                            ...
                            className="text-red-300 hover:text-red-200 p-1 hover:bg-red-500/20 rounded transition-colors cursor-pointer text-sm"
                            title="Remove file"
                          >
                            🗑️
                          </button>
                  ...
                  {!uploadedFile && (
                    <div className="mt-2 flex items-center justify-start gap-2 p-2 rounded-lg text-xs font-medium"
                         style={{
                      ...
                      >
                        <div className={`w-3 h-3 bg-white rounded-full transition-transform ${
                          uploadLater ? 'translate-x-4' : 'translate-x-0.5'
                        }`} />
                      </button>
                      <label className={`text-xs font-medium ${uploadedFile ? 'text-white/50' : 'text-purple-200'}`}>
                        Upload Later
                      </label>
                    </div>
                  )}
                  {uploadLater && !uploadedFile && (
                    <div className="mt-1 text-white/80 text-xs italic flex items-center">
                      <span role="img" aria-label="caution" className="mr-1">
                        ⚠️
                      </span>
                      Submit artwork within 48hrs
                    </div>
                  )}

                  {/* Additional Instructions Input - Separate from Upload Later */}
                  <div className="mt-3">
                    <input
                      type="text"
                      ...
                      className="w-full px-3 py-2 rounded-lg border border-white/20 bg-white/10 text-white placeholder-white/60 focus:outline-none focus:border-purple-400 backdrop-blur-md button-interactive text-sm"
                    />
                  </div>
                </div>
              </div>
            ...
            {/* Right Column - Quantity Selection */}
            <div className="container-style p-3 lg:p-4 transition-colors duration-200">
