...
interface ClearStickerCalculatorProps {
...
export default function ClearStickerCalculator({ initialBasePricing, realPricingData }: ClearStickerCalculatorProps) {
    ...
    const loadReorderData = () => {
      try {
        ...
        if (reorderDataString) {
          console.log('🔄 Loading reorder data for clear stickers...');
  ...
  const calculatePrice = (qty: number, area: number, isRushOrderParam: boolean, vibrancyBoost: boolean = false) => {
    ...
    if (realPricingData && realPricingData.basePricing && realPricingData.quantityDiscounts) {
      ...
      // Apply 15% price increase for clear stickers
      const clearMultiplier = 1.15;
      let adjustedTotal = realResult.totalPrice * whiteOptionMultiplier * clearMultiplier;
      let adjustedPerSticker = realResult.finalPricePerSticker * whiteOptionMultiplier * clearMultiplier;
  ...
  const createCartItem = () => {
    ...
    return {
      ...
      product: {
        ...
        sku: "SS-CL-001",
        name: "Clear Stickers",
        category: "clear-stickers" as const,
        description: "Custom clear stickers with transparent background and vibrant colors",
        shortDescription: "Clear stickers with crystal transparency",
  ...
  .animate-glow-purple {
    box-shadow: 0 0 7px rgba(168, 85, 247, 0.4), 0 0 12px rgba(168, 85, 247, 0.2);
  }
  
  .animate-glow-green {
    box-shadow: 0 0 7px rgba(34, 197, 94, 0.4), 0 0 12px rgba(34, 197, 94, 0.2);
  }
  
  .animate-glow-yellow {
    box-shadow: 0 0 7px rgba(234, 179, 8, 0.4), 0 0 12px rgba(234, 179, 8, 0.2);
  }
  
  .animate-glow-red {
    box-shadow: 0 0 7px rgba(239, 68, 68, 0.4), 0 0 12px rgba(239, 68, 68, 0.2);
  }

  .animate-glow-blue {
    box-shadow: 0 0 7px rgba(59, 130, 246, 0.4), 0 0 12px rgba(59, 130, 246, 0.2);
  }
  
  .animate-glow-orange {
    box-shadow: 0 0 7px rgba(249, 115, 22, 0.4), 0 0 12px rgba(249, 115, 22, 0.2);
  }
                  ...
                  return (
                    <div key={amount} className="relative">
                      <button
                        ...
                        `}
                        style={{
                          border: isSelected && !isGoldTier ? '1.5px solid rgba(34, 197, 94, 0.5)' : undefined
                        }}
                      >
            ...
            <div className="container-style p-4 lg:p-6 transition-colors duration-200">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4 flex flex-col">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    ...
                    <button
                      ...
                      style={{
                        border: uploadLater && !uploadedFile ? '1.5px solid rgba(59, 130, 246, 0.5)' : undefined
                      }}
                    >
                      <div className="flex flex-col items-center gap-2">
                        <span className="text-xl">📤</span>
                        <span className={`text-xs font-medium ${uploadLater && !uploadedFile ? 'text-blue-200' : 'text-white/60'}`}>
                          Upload later
                        </span>
                      </div>
                    </button>
                  ...
                  <div className="flex-grow flex flex-col">
                    <div className={`p-3 rounded-xl backdrop-blur-md transition-all duration-200 flex flex-col flex-grow ${
                      ...
                      <textarea
                        ...
                        className="w-full flex-grow rounded-xl border-0 p-0 resize-none bg-transparent text-white placeholder-white/60 focus:outline-none transition-all appearance-none overflow-hidden"
                        placeholder={isMobile ? "Special instructions for your order..." : "Any special instructions for your order..."}
                        style={{ 
                ...
                <div className="relative">
                  ...
                  {!uploadedFile ? (
                    <div 
                      className={`border-[1.5px] border-solid border-gray-400/20 rounded-xl p-8 text-center transition-all cursor-pointer backdrop-blur-md relative ${
                        uploadedFile 
                          ? 'border-green-400/50 bg-green-500/10 animate-glow-green' 
                          : 'hover:border-gray-400/30 animate-subtle-pulse'
                      }`}
                      onDrop={handleDrop}
