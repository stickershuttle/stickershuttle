...
interface GlitterStickerCalculatorProps {
...
export default function GlitterStickerCalculator({ initialBasePricing, realPricingData }: GlitterStickerCalculatorProps) {
  ...
  }, [selectedSize, customWidth, customHeight, selectedQuantity, customQuantity, selectedWhiteOption, isRushOrder])
    ...
    const loadReorderData = () => {
      try {
        ...
        if (reorderDataString) {
          console.log('🔄 Loading reorder data for glitter stickers...');
  ...
  const calculatePrice = (qty: number, area: number, isRushOrderParam: boolean) => {
    ...
    if (realPricingData && realPricingData.basePricing && realPricingData.quantityDiscounts) {
      ...
      // Apply 15% price increase for glitter stickers
      const glitterMultiplier = 1.15;
      const adjustedTotal = realResult.totalPrice * whiteOptionMultiplier * glitterMultiplier;
      const adjustedPerSticker = realResult.finalPricePerSticker * whiteOptionMultiplier * glitterMultiplier;
  ...
  const createCartItem = () => {
    ...
    return {
      ...
      product: {
        ...
        category: "glitter-stickers" as const,
        description: "Custom glitter stickers with dazzling sparkle finish",
        shortDescription: "Glitter stickers with eye-catching sparkle",
    ...
    <div className="transition-colors duration-200">
      ...
      <div className="">
        ...
        <div className="rounded-3xl">
          ...
          {/* White Options Section - Desktop Only */}
                ...
                <div className="space-y-4">
                    ...
                    {postToInstagram && (
                      <div className="mt-4">
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                          We'll post your sticker on our Instagram and tag you! (Optional)
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <textarea
                      ...
                      placeholder="Any additional instructions or requests? (Optional)"
                      className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:border-purple-400 resize-none"
                      style={{ minHeight: '50px' }}
                      maxLength={500}
                    />
                    <div className="flex justify-between items-center mt-1">
                      <p className="text-xs text-gray-400">
                        Tell us about colors, finishes, or any special requests
                      </p>
