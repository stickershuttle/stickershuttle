...
export default function AIFileImage({
  ...
  return (
    <div className={`relative ${className}`} onClick={handleClick}>
      ...
      {/* Error State */}
      {error && (
        <div className="w-full h-full flex flex-col items-center justify-center bg-white/5 rounded text-gray-400">
      ...
      {isDesign && (
        <div className="absolute bottom-1 left-1 w-3 h-3 bg-blue-500 rounded-full opacity-80"></div>
      )}
    </div>
  );
}
