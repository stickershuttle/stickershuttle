...
export default function BannershipXBanners() {
  ...
  const [noFrame, setNoFrame] = useState<boolean>(false);
  const [postToInstagram, setPostToInstagram] = useState<boolean>(false);
  const [instagramUsername, setInstagramUsername] = useState<string>('');
  const [showInfoTooltip, setShowInfoTooltip] = useState(false);
  const [showNoFrameTooltip, setShowNoFrameTooltip] = useState(false);

  // X Banner preview image
  const xBannerImage = '/x-banner-icon.png?v=2';

  // Get size dimensions in inches - X Banner has one standard size
  const getSizeDimensions = () => {
    return { width: 25, height: 64 }; // Standard X Banner size
  };
  ...
  // X Banner stand cost - single fixed price
  const getBaseFrameCost = () => {
    return 34.13; // Standard X Banner stand
  };

  // Apply or remove stand cost based on "no frame" option
  const getFrameCost = () => {
    if (noFrame) {
      return 0; // No stand cost if user doesn't need the frame
    }
    return getBaseFrameCost();
  };
  ...
  const calculateTotal = (qty?: number) => {
    const quantity = qty !== undefined ? qty : selectedQuantity;
    const standCost = getFrameCost();
    const materialCost = getMaterialCost();
    const sqFt = calculateSquareFootage();
    const unitPrice = standCost + materialCost;
  ...
  // Create cart item for X banners
  const createCartItem = () => {
    ...
    return {
      ...
      product: {
        ...
        sku: "BS-XB-CUSTOM",
        name: "X Banners",
        category: "x-banners" as ProductCategory,
        description: "Professional X-stand banners perfect for trade shows, events, and business displays",
        shortDescription: "Portable X-stand display banners",
        basePrice: pricing.unitPrice,
        pricingModel: "per-unit" as const,
        images: [xBannerImage],
        defaultImage: xBannerImage,
        features: [],
        attributes: [],
        customizable: true,
        isActive: true,
      ...
      customization: {
        ...
        selections: {
          size: {
            ...
            value: `${width}" × ${height}"`,
            displayValue: `Standard (25" × 64")`,
            priceImpact: 0
          },
          includeStand: {
            type: "finish" as const,
            value: !noFrame,
            displayValue: noFrame ? "Banner Only (No Stand)" : "With Stand",
            priceImpact: noFrame ? -34.13 : 0
          },
          rush: {
            type: "finish" as const,
            value: rushOrder,
    ...
    if (typeof window !== 'undefined' && window.fbq) {
      try {
        window.fbq('track', 'ViewContent', {
          content_ids: ['x-banners'],
          content_name: 'X Banners',
          content_category: 'Banners',
          content_type: 'product'
        });
        console.log('📊 Facebook Pixel: ViewContent tracked for X Banners');
      } catch (fbError) {
          ...
          >
            <div className="text-center relative z-10">
              ...
              <p className="text-gray-300 text-sm">
                Professional X-stand display banners. Perfect for events and promotions. Banners can be easily swapped out with multiple designs.
              </p>
            </div>
          </div>
        </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-start gap-3 p-3 rounded-lg text-sm font-medium relative"
                      ...
                      <button
                        onClick={() => setNoFrame(!noFrame)}
                        title={noFrame ? "Include frame" : "Remove frame"}
                        className={`w-12 h-6 rounded-full transition-colors ${
                          noFrame ? 'bg-blue-500' : 'bg-white/20'
                        }`}
                      >
                        <div className={`w-4 h-4 bg-white rounded-full transition-transform ${
                          noFrame ? 'translate-x-7' : 'translate-x-1'
                        }`} />
                      </button>
                      <div className="flex-1">
                        <label className="text-sm font-medium text-white">
                          🖼️ I don't need the frame
                        </label>
                      </div>
                      {/* Info Icon */}
                      <div className="relative">
                        <span 
                          className="text-blue-300 cursor-pointer text-sm font-medium select-none hover:text-blue-200 transition-colors"
                          onClick={() => setShowNoFrameTooltip(!showNoFrameTooltip)}
                          onMouseEnter={() => setShowNoFrameTooltip(true)}
                          onMouseLeave={() => setShowNoFrameTooltip(false)}
                        >
                          ⓘ
                        </span>
                        {showNoFrameTooltip && (
                          <div className="absolute right-0 sm:right-auto sm:left-1/2 sm:-translate-x-1/2 bottom-full mb-2 z-50 w-72 sm:w-64">
                            <div className="text-white text-xs rounded-lg px-3 py-2 whitespace-normal" style={{
                              ...
                              Select this option if you already have an X banner stand and just need a new banner to replace your current one.
              ...
              <div className="container-style p-6 transition-colors duration-200">
                ...
                {pricing.total > 0 ? (
                  <div className="space-y-2 text-sm">
                    {/* Stand Cost - Only show if frame is included */}
                    {!noFrame && (
                      <div className="flex justify-between items-center py-2">
                        <span className="text-gray-300">
                          Stand Cost{selectedQuantity > 1 ? ` (×${selectedQuantity})` : ''}
                        </span>
                        <span className="text-white font-medium">${(pricing.standCost * selectedQuantity).toFixed(2)}</span>
                      </div>
                    )}
                    ...
                    {noFrame && (
                      <div className="flex justify-between items-center py-2">
                        <span className="text-blue-400">Banner Only (No Stand){selectedQuantity > 1 ? ` (×${selectedQuantity})` : ''}</span>
                        <span className="text-blue-400 font-medium">-${(34.13 * selectedQuantity).toFixed(2)}</span>
                      </div>
                    )}
